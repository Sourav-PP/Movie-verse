...
export class FavoriteFileRepository implements IFavoriteRepository {
    ...
    private async readFile(): Promise<MovieSummary[]> {
        try {
            ...
            return JSON.parse(data) as MovieSummary[];
        } catch (error) {
            throw new AppError(messages.ERROR.READING_FILE)
        }
    }

    async writeFile(movies: MovieSummary[]): Promise<void> {
    ...
    async addFavorites(movie: MovieSummary): Promise<void> {
